...
st.sidebar.markdown(
    """
    # Dashboard Overview and Scenario Details

    This dashboard provides a comprehensive framework for macroeconomic and financial forecasting. It is built to support strategic decision-making by evaluating both historical performance and forward-looking stress testing scenarios.

    ## Key Components:
    **Data Upload & Preprocessing:**  
      -Upload monthly time-series data, which is then standardized with consistent frequency and date formatting.
     ...
     **What-If Scenarios:**  
      -Allows users to manually input values for independent variables to simulate forecasts. It provides one‑step forecasts from each selected model (or an ensemble), so that you can analyze potential outcomes under different assumptions.

     **Scenario Forecast:**  
      -In this tab, users upload multiple Excel files containing scenario data. The dashboard then applies the chosen forecasting model to each scenario and compares the forecasts against historical data. Blending options are available (for models like VAR and VECM) to mix historical trends with the scenario inputs. This tab is focused on comparing different economic scenarios side-by-side.

     **Stress Testing (Basic):**  
      -Uses key macroeconomic parameters (e.g., GDP growth, unemployment rate, interest rate) to compute stressed values for exposure (EAD), loss given default (LGD), and probability of default (PD). It then calculates an aggregated expected loss, offering a high-level risk snapshot.

     **Stress Testing 2:**  
      -Leverages the best performing forecasting model (determined in Model Selection) to generate forecasts from scenario data. Models such as SARIMAX, OLS, VAR, VECM, ECM, and VARX are used with blending options. Forecasts are multiplied by stressed parameters to compute expected losses. This tab provides detailed risk metrics and visualizations to help identify periods of high, moderate, or low risk.

     **Stress Testing 3:**  
      -Focuses on a more granular, detailed stress test using individual model forecasts (excluding ensemble/hybrid models). In addition to using models like VAR, VECM, SARIMAX, and OLS, it includes a dedicated ECM branch. For ECM, a cointegration relationship is first estimated to compute the error correction term (ECT), and then short-run adjustments are forecasted by comparing the scenario inputs against the last observed levels. The expected losses are calculated at a detailed level, and results are presented with risk color-coding and comprehensive insights.

    ## Strategic Implications:
     **Risk Identification & Capital Adequacy:**  
      -The dashboard helps to identify periods of potential risk under adverse scenarios, allowing risk managers to adjust capital buffers and diversify assets.

     **Portfolio Optimization:**  
      -Forecast comparisons and stress test outcomes provide guidance on adjusting lending strategies, asset allocations, and hedging strategies.

     **Model Validation & Continuous Improvement:**  
      -Backtesting and multiple performance metrics ensure that the models capture underlying economic relationships accurately, while ensemble options help reduce uncertainty.

    This comprehensive overview ensures that users not only know what each part of the dashboard does but also understand how the different scenario and stress testing tabs complement one another to deliver actionable financial insights.
    """
    ...
    if st.session_state["data"] is not None:
        ...
        try:
            ...
            if selection_mode == "Automatic (Model-Based)":
                selected_model = st.selectbox("Select Model for Feature Selection",
                                              list(model_specific_features.keys()),
                                              key="automatic_model_select")
                if selected_model in ["LASSO", "Ridge", "Elastic Net"]:
                    if selected_model == "LASSO":
                        lasso_alpha = st.slider("LASSO Alpha", min_value=0.0001, max_value=1.0, value=0.01, step=0.0001,
                                                key="lasso_alpha")
                        try:
                            from sklearn.linear_model import Lasso
                        ...
                        ridge_alpha = st.slider("Ridge Alpha", min_value=0.0001, max_value=1.0, value=0.01, step=0.0001,
                                                key="ridge_alpha")
                        try:
                            from sklearn.linear_model import Ridge
                        ...
                        elasticnet_alpha = st.slider("ElasticNet Alpha", min_value=0.0001, max_value=1.0, value=0.01,
                                                     step=0.0001, key="elasticnet_alpha")
                        elasticnet_l1_ratio = st.slider("ElasticNet L1 Ratio", min_value=0.0, max_value=1.0, value=0.5,
                                                        step=0.01, key="elasticnet_l1_ratio")
                        try:
                            from sklearn.linear_model import ElasticNet
                ...
                sorted_importances = np.array(feature_importances[selected_model])[
                                         np.argsort(feature_importances[selected_model])[::-1]][:num_features]
                ...
                for feature in selected_top_features:
                    if feature in independent_vars:
                        ...
                        sorted_importances.append(
                            np.mean([feature_importances[m][feature_idx] for m in feature_importances.keys()]))
                    else:
            ...
            if selected_top_features:
                importance_df = pd.DataFrame(
                    {"Feature": selected_top_features, "Importance": sorted_importances}).sort_values(by="Importance",
                                                                                                      ascending=True)
                plt.figure(figsize=(10, 6))
                plt.barh(importance_df["Feature"], importance_df["Importance"],
                         color=plt.cm.viridis(np.linspace(0, 1, len(importance_df))))
                plt.title(f"Feature Importances ({'Manual' if selection_mode == 'Manual' else selected_model})",
        ...
        try:
            ...
            lime_explainer = LimeTabularExplainer(training_data=X.values, feature_names=X.columns.tolist(),
                                                  mode="regression")
            ...
            lime_exp = lime_explainer.explain_instance(selected_instance.values, rf_model.predict,
                                                       num_features=len(selected_features))
            lime_importances = dict(lime_exp.as_list())
            importance_df = pd.DataFrame(
                {"Feature": list(lime_importances.keys()), "Importance": list(lime_importances.values())}).sort_values(
        ...
        try:
            ...
            if selected_model == "SARIMAX":
                ...
                try:
                    ...
                    X_ecm_reg = sm.add_constant(
                        data_diff[["lagECT"] + selected_features + ["lag_d_" + feat for feat in selected_features]])
        ...
        if uploaded_scenarios:
            ...
            for scenario_file in uploaded_scenarios:
                ...
                if not set(st.session_state["selected_features"]).issubset(scenario_df.columns):
                    st.error(
                        f"The file {scenario_file.name} must contain the following columns: {st.session_state['selected_features']}")
                    continue
                    ...
                    # Ensemble branch: Average forecasts from available methods
                        ...
                        try:
                            ...
                            ensemble_forecasts.append(
                                sarimax_model.get_forecast(steps=1, exog=input_df).predicted_mean.iloc[0])
                        except Exception:
                            pass
                        try:
                            from statsmodels.api import OLS, add_constant
                        ...
                        try:
                            ...
                            var_fit = var_model.fit(maxlags=2, ic='aic')
                            p = var_fit.k_ar
                            historical_last = st.session_state["data"][st.session_state["selected_features"]].values[
                                              -p:].copy()
                            scenario_input = input_df.reindex(columns=st.session_state["selected_features"])
                            scenario_input = scenario_input.fillna(pd.DataFrame(
                                st.session_state["data"][st.session_state["selected_features"]]).mean()).values[0]
                        ...
                        try:
                            ...
                            if cointegration_rank > 0:
                                new_data = st.session_state["data"].copy()
                                scenario_input = \
                                input_df.reindex(columns=new_data.columns).fillna(new_data.mean()).values[0]
                                blended = (1 - ens_vecm_blend) * new_data.iloc[
                                    -1].values + ens_vecm_blend * scenario_input
...
#######################################
    ...
    if "scenario_data" in st.session_state:
        ...
        gdp_growth_value = st.number_input(f"Enter Value for {gdp_growth_var}",
                                           value=float(selected_scenario[gdp_growth_var].iloc[0]),
                                           key="gdp_growth_value")
        unemployment_rate_value = st.number_input(f"Enter Value for {unemployment_rate_var}",
                                                  value=float(selected_scenario[unemployment_rate_var].iloc[0]),
                                                  key="unemployment_rate_value")
        interest_rate_value = st.number_input(f"Enter Value for {interest_rate_var}",
                                              value=float(selected_scenario[interest_rate_var].iloc[0]),
        ...
        if result_type == "Aggregated":
            ...
            st.write(
                f"The expected loss of **{aggregated_expected_loss:.2f} million** is derived from the stressed parameters:")
            ...
            try:
                ...
                if st.button("Show Expected Loss Plot"):
                    ...
                    plt.axhline(y=aggregated_expected_loss, color='red', linestyle='--',
                                label=f'Baseline Risk Line: {aggregated_expected_loss:.2f} million')
                    plt.xlabel('Month')
                    ...
                    plt.fill_between(results_df.index, expected_losses, aggregated_expected_loss,
                                     where=(expected_losses > aggregated_expected_loss), color='red', alpha=0.3,
                                     label='Risk Underestimation Zone / High-Risk Zone')
                    plt.fill_between(results_df.index, expected_losses, aggregated_expected_loss,
                                     where=(expected_losses < aggregated_expected_loss), color='green', alpha=0.3,
                                     label='Risk Overestimation Zone / Low-Risk Zone')
                    plt.text(results_df.index[-1], aggregated_expected_loss + 5,
                             'Risk Underestimation Zone / High-Risk Zone', color='black', fontsize=10,
                             verticalalignment='bottom', horizontalalignment='right')
                    plt.text(results_df.index[-16], aggregated_expected_loss - 5,
                             'Risk Overestimation Zone / Low-Risk Zone', color='black', fontsize=10,
                ...
                moderate_risk_periods = results_df[
                    (results_df["Expected Loss"] > 50) & (results_df["Expected Loss"] <= 100)]
...
##################################
    ...
    probability_default = st.number_input("Enter Probability of Default (PD) (as a percentage)", value=5.0, step=0.1,
                                          key="pd_3") / 100
    if "valid_models" in st.session_state and st.session_state["valid_models"]:
        valid_models = st.session_state["valid_models"]
        ...
        if filtered_models:
            ...
            if "scenario_data" in st.session_state:
                ...
                gdp_growth_var = st.selectbox("Select Variable for GDP Growth", options=available_variables,
                                              key="st3_gdp")
                unemployment_rate_var = st.selectbox("Select Variable for Unemployment Rate",
                                                     options=available_variables, key="st3_unemp")
                interest_rate_var = st.selectbox("Select Variable for Interest Rate", options=available_variables,
                                                 key="st3_ir")
                input_data = selected_scenario[[gdp_growth_var, unemployment_rate_var, interest_rate_var]].copy()
                for feature in st.session_state["selected_features"]:
                ...
                if selected_model_name == "VAR":
                    ...
                    forecast = var_results.forecast(st.session_state["data"].values[-var_results.k_ar:],
                    ...
                    cointegration_rank = sum(jres.lr1 > crit)
                    if cointegration_rank > 0:
                        vecm_blend = st.slider("Blend Weight (VECM)", min_value=0.0, max_value=1.0, value=0.5,
                                               step=0.01, key="vecm_blend_st3")
                        new_data = st.session_state["data"].copy()
                        forecasts_list = []
                        for i in range(len(input_data)):
